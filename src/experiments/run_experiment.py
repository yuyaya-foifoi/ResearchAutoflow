...
def run_experiment(
    ...
    s3_upload_enabled = all(
        [bucket, s3_folder, access_key, secret_key, region]
    )
    ...
    for iteration in range(num_experiment):
        ...
        critic_message = critic(
            code, code_desc, experiment_explanation, survey_arg.to_string()
        )
        experiment_log += (
            "-" * 25 + f"\n批評家からのコメント:\n{critic_message}\n"
        )
        ...
        query = generate_query(idea)

        # 論文解析
        try:
            survey_df = analyze_papers_to_df(query.idea)
        ...
        with open(
            os.path.join(f"iteration_{iteration+1}", "slide.html"),
            "w",
            encoding="utf-8",
        ) as file:
